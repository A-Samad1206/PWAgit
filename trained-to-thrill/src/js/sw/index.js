...
var expectedCaches = [staticCacheName, 'trains-imgs', 'trains-data'];
    ...
    caches.keys().then(function (cacheNames) {
      return Promise.all(
        cacheNames.map(function (cacheName) {
          if (
            /^trains-/.test(cacheName) &&
            expectedCaches.indexOf(cacheName) == -1
          ) {
...
function flickrAPIResponse(request) {
  if (request.headers.get('x-use-cache-only')) {
    ...
    return flickrAPIResponse(new Request(request, { headers: headers }))
      .then(function (response) {
        return response.json();
      })
      .then(function (data) {
        var imgRequests = data.photos.photo
          .map(getPhotoURL)
          .map(function (url) {
    ...
    return fetch(request).then(function (response) {
      return caches.open('trains-data').then(function (cache) {
        ...
        Promise.all([response.clone().json(), caches.open('trains-imgs')]).then(
          function (results) {
            var data = results[0];
